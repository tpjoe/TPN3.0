...
class BRCausalModel(TimeVaryingCausalModel):
    ...
    def training_step(self, batch, batch_ind, optimizer_idx=0):
        ...
        if optimizer_idx == 0:  # grad reversal or domain confusion representation update
            ...
            # Fourth head loss: binary classification with treatment
            # Now binary_treatment_pred has shape (batch_size, seq_len, n_buckets)
            # We only use the last bucket (index -1) for loss calculation
            if binary_treatment_pred.dim() == 3:
                # Take last timestep and last bucket
                binary_treatment_pred_last = binary_treatment_pred[:, -1, 1]  # Use bucket 1 (eventual), not censored
            else:
                # Should not happen with current architecture
                binary_treatment_pred_last = binary_treatment_pred[:, -1]
            
            binary_treatment_bce_loss = F.binary_cross_entropy_with_logits(
                binary_treatment_pred_last,
                binary_labels,
                reduction='mean'
            )
            ...
            if hasattr(self.hparams.dataset, 'use_gradnorm') and self.hparams.dataset.use_gradnorm:
                ...
                # Equal weights for all three heads (muting lambda for now)
                outcome_combined = (outcome_loss + binary_bce_loss + binary_treatment_bce_loss) / 3.0
            ...
            self.log(f'{self.model_type}_train_binary_treatment_bce_loss', binary_treatment_bce_loss, on_epoch=True, on_step=False, sync_dist=True)
            ...
            if self.hparams.exp.weights_ema:
                with self.ema_non_treatment.average_parameters():
                    treatment_pred, _, _, _, _ = self(batch, detach_treatment=True)
            else:
                treatment_pred, _, _, _, _ = self(batch, detach_treatment=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Fourth head loss
        if binary_treatment_pred.dim() == 3:
            binary_treatment_pred_last = binary_treatment_pred[:, -1, -1]
        else:
            binary_treatment_pred_last = binary_treatment_pred[:, -1]
        ...
        self.log('val_binary_treatment_bce_loss', binary_treatment_bce_loss, on_epoch=True, prog_bar=False, sync_dist=True)
    ...
    def calculate_binary_metrics(self, dataset, dataset_name='val'):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader):
                ...
                # Get predictions
                _, _, _, binary_pred, binary_treatment_pred = self(batch)
                ...
                # Same for binary_treatment_pred but use dimension 1 (eventual bucket)
                if binary_treatment_pred.dim() == 3:
                    # Take last timestep and dimension 1 (eventual bucket, not censored)
                    binary_treatment_probs = torch.sigmoid(binary_treatment_pred[:, -1, 1])
                else:
                    # Should not happen with current architecture
                    binary_treatment_probs = torch.sigmoid(binary_treatment_pred[:, -1])
    ...
    def calculate_bucket_specific_metrics(self, dataset, dataset_name='val'):
        """Calculate bucket-specific metrics for the fourth head only
        
        This evaluates how well each bucket predicts events in its specific time window,
        with exclusion rules for cascading predictions.
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader):
                ...
                # Get predictions (only need fourth head)
                _, _, _, _, binary_treatment_pred = self(batch)
                ...
                bucket0_labels = torch.zeros(batch_size)
                bucket1_labels = torch.zeros(batch_size)
                ...
                for i in range(batch_size):
                    ...
                    if event_indicator[i].max() > 0:
                        ...
                        if len(event_indices) > 0:
                            ...
                            # Bucket 0 label: 1 if event in bucket 0
                            if event_bucket == 0:
                                bucket0_labels[i] = 1
                                bucket1_labels[i] = -1  # Exclude from bucket 1
                            # Bucket 1 label: 1 if event in bucket 1
                            elif event_bucket == 1:
                                bucket0_labels[i] = 0
                                bucket1_labels[i] = 1
                            else:
                                # Event in later bucket or censored bucket
                                bucket0_labels[i] = 0
                                bucket1_labels[i] = 0
                    else:
                        # Censored patient - negative for both buckets
                        bucket0_labels[i] = 0
                        bucket1_labels[i] = 0
                ...
                if binary_treatment_pred.dim() == 3:
                    # Apply sigmoid to convert hazards to probabilities
                    bucket_probs = torch.sigmoid(binary_treatment_pred[:, -1, :])
                    bucket0_probs = bucket_probs[:, 0]
                    bucket1_probs = bucket_probs[:, 1]
                else:
                    # Should not happen with current architecture
                    raise ValueError(f"Unexpected prediction shape: {binary_treatment_pred.shape}")
                
                all_bucket0_labels.append(bucket0_labels.cpu())
                all_bucket1_labels.append(bucket1_labels.cpu())
                all_bucket0_preds.append(bucket0_probs.cpu())
                all_bucket1_preds.append(bucket1_probs.cpu())
                all_person_ids.append(person_ids.cpu())
                all_original_labels.append(original_labels.cpu())
        
        # Concatenate all batches
        all_bucket0_labels = torch.cat(all_bucket0_labels).numpy()
        all_bucket1_labels = torch.cat(all_bucket1_labels).numpy()
        all_bucket0_preds = torch.cat(all_bucket0_preds).numpy()
        all_bucket1_preds = torch.cat(all_bucket1_preds).numpy()
        ...
        # Bucket 0 metrics (immediate: 0-1 days)
        n_bucket0_events = int((all_bucket0_labels == 1).sum())
        n_bucket0_controls = int((all_bucket0_labels == 0).sum())
        
        if n_bucket0_events > 0 and n_bucket0_controls > 0:
            try:
                bucket0_auc_roc = roc_auc_score(all_bucket0_labels, all_bucket0_preds)
                bucket0_auc_pr = average_precision_score(all_bucket0_labels, all_bucket0_preds)
            except:
                bucket0_auc_roc = np.nan
                bucket0_auc_pr = np.nan
        else:
            bucket0_auc_roc = np.nan
            bucket0_auc_pr = np.nan
        
        metrics[f'{dataset_name}_bucket0_auc_roc'] = bucket0_auc_roc
        metrics[f'{dataset_name}_bucket0_auc_pr'] = bucket0_auc_pr
        metrics[f'{dataset_name}_bucket0_n_events'] = n_bucket0_events
        metrics[f'{dataset_name}_bucket0_n_controls'] = n_bucket0_controls
        metrics[f'{dataset_name}_bucket0_prevalence'] = n_bucket0_events / len(all_bucket0_labels)
        
        # Bucket 1 metrics (eventual: 2+ days) - with exclusions
        # Only evaluate samples that weren't excluded (didn't have event in bucket 0)
        bucket1_mask = all_bucket1_labels != -1
        bucket1_labels_at_risk = all_bucket1_labels[bucket1_mask]
        bucket1_preds_at_risk = all_bucket1_preds[bucket1_mask]
        
        n_bucket1_at_risk = len(bucket1_labels_at_risk)
        n_bucket1_events = int((bucket1_labels_at_risk == 1).sum())
        n_bucket1_controls = int((bucket1_labels_at_risk == 0).sum())
        
        if n_bucket1_events > 0 and n_bucket1_controls > 0:
            try:
                bucket1_auc_roc = roc_auc_score(bucket1_labels_at_risk, bucket1_preds_at_risk)
                bucket1_auc_pr = average_precision_score(bucket1_labels_at_risk, bucket1_preds_at_risk)
            except:
                bucket1_auc_roc = np.nan
                bucket1_auc_pr = np.nan
        else:
            bucket1_auc_roc = np.nan
            bucket1_auc_pr = np.nan
        
        metrics[f'{dataset_name}_bucket1_auc_roc'] = bucket1_auc_roc
        metrics[f'{dataset_name}_bucket1_auc_pr'] = bucket1_auc_pr
        metrics[f'{dataset_name}_bucket1_n_events'] = n_bucket1_events
        metrics[f'{dataset_name}_bucket1_n_controls'] = n_bucket1_controls
        metrics[f'{dataset_name}_bucket1_n_at_risk'] = n_bucket1_at_risk
        metrics[f'{dataset_name}_bucket1_n_excluded'] = int((all_bucket1_labels == -1).sum())
        metrics[f'{dataset_name}_bucket1_prevalence'] = n_bucket1_events / n_bucket1_at_risk if n_bucket1_at_risk > 0 else 0
    ...
    def calculate_treatment_pearson_r(self, dataset, dataset_name='val'):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader):
                ...
                # Get predictions
                treatment_pred, _, _, _, _ = self(batch)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log('test_binary_treatment_bce_loss', binary_treatment_bce_loss, on_epoch=True, prog_bar=False, sync_dist=True)
    ...
    def predict_step(self, batch, batch_idx, dataset_idx=None):
        ...
        if self.hparams.exp.weights_ema:
            with self.ema_non_treatment.average_parameters():
                _, outcome_pred, br, _, _ = self(batch)
        else:
            _, outcome_pred, br, _, _ = self(batch)
